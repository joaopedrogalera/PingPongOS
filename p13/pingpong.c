...
void dispatcher_body(void* args){
  ...
  while(readyTasks!=NULL||sleepingTasks!=NULL||(disk.diskQueue)!=NULL||disk.suspend!=NULL){
...
int mqueue_send (mqueue_t *queue, void *msg){
  mqueue_item_t *item;
  item = malloc(sizeof(mqueue_item_t));
  item->cont = malloc(queue->size);
  item->prev = NULL;
  item->next = NULL;

  memcpy(item->cont,msg,queue->size);
  ...
  queue_append((queue_t**) &(queue->buffer),(queue_t*) item);
...
int mqueue_recv (mqueue_t *queue, void *msg){
  ...
  item = (mqueue_item_t*) queue_remove((queue_t**) &(queue->buffer), (queue_t*) queue->buffer);
  sem_up(&(queue->s_buffer));
  sem_up(&(queue->s_vaga));
  memcpy(msg,item->cont,queue->size);
  free(item->cont);
  free(item);
...
int mqueue_msgs (mqueue_t *queue){
  ...
  sem_down(&(queue->s_buffer));
  i = queue_size((queue_t*)queue->buffer);
  sem_up(&(queue->s_buffer));
  return(i);
}
...
void diskDriverBody(void *arg){
  while(1){
    ...
    if((disk.sig)==1){
      disk.sig = 0;
      queue_append((queue_t**) &readyTasks, (queue_t*) disk.suspend);
      disk.suspend = NULL;
    }

    if((disk_cmd(DISK_CMD_STATUS, 0, 0)==1)&&(disk.diskQueue)!=NULL){
      disk.suspend = (task_t*) queue_remove((queue_t**) &(disk.diskQueue),(queue_t*) (disk.diskQueue));
      if((disk.suspend->diskOp)==1){
        disk_cmd (DISK_CMD_READ, disk.suspend->diskBlock, disk.suspend->diskBuffer);
      }
      else{
        disk_cmd (DISK_CMD_WRITE, disk.suspend->diskBlock, disk.suspend->diskBuffer);
      }
    }
    sem_up(&(disk.sDisk));
    task_switch(&taskDispatcher);
...
void diskSig(int signum){
  if(signum == SIGUSR1){
    disk.sig = 1;
    queue_append((queue_t**) &readyTasks,(queue_t*) &(disk.diskDriverTask));
  }
}
...
int diskdriver_init (int *numBlocks, int *blockSize){
  ...
  *blockSize = disk_cmd (DISK_CMD_BLOCKSIZE, 0, 0);
  ...
  disk.sig = 0;
  disk.suspend = NULL;
  sem_create(&(disk.sDisk),1);
  task_create(&(disk.diskDriverTask),diskDriverBody,NULL);

  diskAction.sa_handler = diskSig ;
  sigemptyset (&diskAction.sa_mask) ;
  diskAction.sa_flags = 0 ;
  sigaction (SIGUSR1, &diskAction, 0);
  return(0);
}

int disk_block_read (int block, void *buffer){
  runningTask->diskOp = 1;
  runningTask->diskBlock = block;
  runningTask->diskBuffer = buffer;
  queue_append((queue_t**) &(disk.diskQueue),(queue_t*) runningTask);
  task_switch(&(disk.diskDriverTask));
  return(0);
}

int disk_block_write (int block, void *buffer){
  runningTask->diskOp = 2;
  runningTask->diskBlock = block;
  runningTask->diskBuffer = buffer;
  queue_append((queue_t**) &(disk.diskQueue),(queue_t*) runningTask);
  task_switch(&(disk.diskDriverTask));
